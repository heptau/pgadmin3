...
pgObject *pgSchemaBaseFactory::CreateObjects(pgCollection *collection, ctlTree *browser, const wxString &restriction)
{
	...
	restr += wxT("(nspname LIKE '_%' AND EXISTS (SELECT 1 FROM information_schema.routine_privileges f where f.routine_name='slonyversion' and f.grantee=current_user and f.routine_schema=nspname LIMIT 1))\n");
