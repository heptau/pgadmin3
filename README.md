...
Этот проект является форком pgAdmin3 v1.22. который уже не поддерживается.
Полная версия pgAdmin3 1.22 находиться тут https://github.com/postgres/pgadmin3.git.

Что изменено:
* Были добавлены возможности новых версий PostgreSQL.
* Изменены некоторые элементы интерфейса, добавлены новые возможности выполнения запросов,
  построения планов и обработки результатов запроса, мониторинга серверов и просмотра логов.
* Мониторинг настроен на csv логи.
* Расширенны возможности плагинов (использование putty, far2l, ssh).

Некоторые новые функции требуют наличия расширений amcheck,pg_query_state,pg_wait_sampling,pgpro_pwr.
Многие иконки интерфейса переведены в svg.
**ВАЖНО**: иконки для COMMIT и ROLLBACK изменены,
зеленая это commit, красная rollback (как в PL/SQL Developer).

Многие не стандартные возможности по умолчанию отключены.
Все изменения сведены в changelog.md из commit-ов.

Для удобства работы в Windows последний скомпилированный исполняемый файл будет 
находиться в каталоге Release.
Для работы достаточно заменить оригинальный pgAdmin3.exe.
Будет поддерживаться только оригинальная версия PostgreSQL и PostrgesPro Enterprise.
Поддерживаемые ОС Alt Linux и Windows.
Инструкция для компиляции и установки в INSTALL.txt

# Описание некоторых функций и возможностей:
